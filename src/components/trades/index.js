...
const Trades = (props) => {
  const { trades } = useSelector(state => state.orderBook)
  const _trades = trades && Array.isArray(trades) && trades.length && trades.reduce((acc,item) => {
    const day = item[1]
                  ...
                  <React.Fragment key={`trade-${formatted_date}`}>
                    ...
                    <React.Fragment>
                      {Array.isArray(_trades[k]) && _trades[k].map((i, ix) => {
                        // console.log("asd",i[1],i[2],i[3])
                        if (i[1] !== undefined && i[2] !== undefined && i[3] !== undefined) {
                          const time = i[1] && moment(i[1]).format("hh:mm:ss")

                          const price = i[2] && i[3] && numberWithCommas(i[3])
